// lane_align_task.cpp
// Build: g++ lane_align_task.cpp -O2 -std=gnu++17 -I/usr/include/eigen3 `pkg-config --cflags --libs opencv4` -o lane_align
// Run:   ./lane_align mask.png prior.csv [--roi 10] [--horizon_frac 0.67] [--debug debug.png]
...
static cv::Mat rasterizeROI(const std::vector<Pt> &poly, cv::Size sz, int half_width_px)
{
    ...
    {
        cv::line(roi,
                 cv::Point(poly[i - 1].u, poly[i - 1].v),
                 cv::Point(poly[i].u, poly[i].v),
                 255, 1, cv::LINE_AA);
    }

    // TODO A: thicken into a tube via dilation
    // 1) cv::getStructuringElement(cv::MORPH_ELLIPSE, Size(2*half_width_px+1, 2*half_width_px+1))
    // 2) cv::dilate(roi, roi, kernel)
...
int main(int argc, char **argv)
{
    ...
    if (!debugPath.empty())
    {
        ...
        cv::line(dbg, cv::Point(x1, y1), cv::Point(x2, y2), cv::Scalar(0, 255, 255), 2, cv::LINE_AA);
        // center & eval point
        cv::line(dbg, cv::Point((int)cx, 0), cv::Point((int)cx, H - 1), cv::Scalar(0, 255, 0), 1);
        cv::circle(dbg, cv::Point((int)std::round(x_eval), (int)std::round(y_eval)), 4, cv::Scalar(0, 0, 255), -1);
